...
<template>
  <div class="instrument-view">
    ...
    <div v-else-if="error" class="error-state">
      ...
      <p>{{ error }}</p>
      <button class="back-btn" style="margin-top: 1rem" @click="load">Reintentar</button>
    </div>
...
.metrics-section { display: flex; flex-direction: column; gap: 0.75rem; }
.metrics-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));
  gap: 1rem;
}
.metric-skeleton { height: 90px; border-radius: 0.75rem; }

.chart-section { display: flex; flex-direction: column; gap: 0.25rem; }
